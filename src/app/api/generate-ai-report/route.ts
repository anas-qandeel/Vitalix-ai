...
- سطر «تنبيهات السلامة» ملزم: لا تقترح أي دواء أو مكمّل أو طعام أو مشروب مذكور في حساسية المريض أو مشتق منه، ولا تقترح شيئاً غير آمن أثناء الحمل أو الرضاعة. لا تذكر الحساسية للمريض إلا إن غيّرت نصيحة كنت ستقدمها.
...
### سكر الدم
صائم: أقل من 54 → هبوط طارئ 🔴 | 54–79 (فوق 60 سنة) أو 54–69 (60 سنة أو أقل) → انخفاض 🟡 | 70–99 → طبيعي 🟢 (بحسب العمر) | 100–125 → ارتفاع طفيف 🟡 | 126 فأكثر → ارتفاع ملحوظ 🔴
بعد الأكل أو غير محدد: أقل من 54 → هبوط طارئ 🔴 | أقل من 70 → انخفاض 🟡 | 70–139 → طبيعي 🟢 | 140–179 → ارتفاع نسبي 🟡 | 180 فأكثر → ارتفاع ملحوظ 🔴
عشوائي: أقل من 54 → هبوط طارئ 🔴 | أقل من 70 → انخفاض 🟡 | 70–139 → طبيعي 🟢 | 140–199 → ارتفاع نسبي 🟡 | 200 فأكثر → ارتفاع ملحوظ 🔴 — انتبه: معيار العشوائي أوسع من بعد الأكل، لا تخلط بينهما
  ...
  try {
    ...
    if (geminiApiKey) {
      try {
        ...
        // ── تنبيهات السلامة: حساسية الأدوية/الأطعمة + الحمل/الرضاعة — تصل للاستدعاءين (التقرير وملخص الصيدلاني) عبر userPrompt ──
        const drugAllergies: string[] = Array.isArray(patient?.drug_allergies) ? patient.drug_allergies.filter((x: any) => typeof x === 'string' && x.trim()) : [];
        const foodAllergies: string[] = Array.isArray(patient?.food_allergies) ? patient.food_allergies.filter((x: any) => typeof x === 'string' && x.trim()) : [];
        const safetyParts: string[] = [
          `حساسية الأدوية: ${drugAllergies.length ? drugAllergies.join('، ') : 'لا يوجد مسجّل'}`,
          `حساسية الأطعمة: ${foodAllergies.length ? foodAllergies.join('، ') : 'لا يوجد مسجّل'}`,
        ];
        if (patient?.is_pregnant === true)  safetyParts.push('المريضة حامل');
        if (patient?.is_lactating === true) safetyParts.push('المريضة مرضعة');
        const safetyLine = `تنبيهات السلامة (ملزمة): ${safetyParts.join(' | ')}`;
