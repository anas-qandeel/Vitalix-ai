...
function MedModal({ patientId, pharmacyId, existingMeds, patientName, onClose, onSaved, onBack, isNewPatient, drugAllergies, foodAllergies, isPregnant, isLactating, diagnosedConditions }: {
  ...
  drugAllergies?: string[] | null; // حساسية الأدوية — تنبيه في رأس النافذة
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between gap-3 bg-white shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              {(drugAllergies?.length ?? 0) > 0 && (
                <p className="text-[11px] font-bold text-rose-700 mt-1 truncate">حساسية دواء: {(drugAllergies || []).join('، ')}</p>
              )}
              {(foodAllergies?.length ?? 0) > 0 && (
                <p className="text-[11px] font-bold text-amber-700 mt-0.5 truncate">حساسية طعام: {(foodAllergies || []).join('، ')}</p>
              )}
        ...
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between gap-3 shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              {(card.patient.drug_allergies?.length ?? 0) > 0 && (
                <p className="text-[11px] font-bold text-rose-700 mt-1 truncate">حساسية دواء: {(card.patient.drug_allergies || []).join('، ')}</p>
              )}
            </div>
          </div>
          ...
          {((patient.diagnosed_conditions?.length ?? 0) > 0 || (patient.drug_allergies?.length ?? 0) > 0 || (patient.food_allergies?.length ?? 0) > 0 || patient.is_pregnant || patient.is_lactating) && (
            <div className="flex flex-wrap gap-2">
              ...
              {(patient.food_allergies?.length ?? 0) > 0 && (
                <span className="text-[10px] font-bold text-amber-700 bg-amber-50 border border-amber-200 px-2 py-1 rounded-lg">حساسية طعام: {(patient.food_allergies || []).join('، ')}</span>
              )}
              {patient.is_pregnant && <span className="text-[10px] font-bold text-pink-700 bg-pink-50 border border-pink-200 px-2 py-1 rounded-lg">حامل</span>}
              {patient.is_lactating && <span className="text-[10px] font-bold text-pink-700 bg-pink-50 border border-pink-200 px-2 py-1 rounded-lg">مرضعة</span>}
            </div>
          )}
        ...
        {showStats && total > 0 && (() => {
          ...
          return (
            <div className="space-y-4 saas-slide-up" dir="rtl">
              ...
              {/* ── Hero — النتيجة الأهم ── */}
              <div className="relative bg-white border-2 border-teal-500 rounded-2xl overflow-hidden">
                <div className="relative px-6 py-6 flex items-center justify-between gap-4">
                  <div>
                    <p className="text-teal-600 text-xs font-bold mb-3">ملخص أداء الصيدلية</p>
                    <div className="flex items-baseline gap-3">
                      <p className="text-6xl sm:text-7xl font-black tabular-nums leading-none text-slate-900">{renewed.length}</p>
                      <p className="text-slate-700 text-base font-semibold">مريض جدّد أدويته من صيدليتك</p>
                    </div>
                    <p className="text-slate-400 text-xs mt-2">نتيجة متابعتك المنتظمة ومبادرتك بالتواصل</p>
                  </div>
                  <div className="shrink-0 hidden sm:flex w-20 h-20 rounded-2xl bg-teal-50 items-center justify-center">
              ...
              {/* ── معدل التحويل — رسم دائري ── */}
              {totalMessaged > 0 && (
                <div className="bg-white border border-slate-200 rounded-xl p-5">
                  <p className="text-sm font-bold text-slate-800 mb-4">رحلة المريض من التذكير للتجديد</p>
                  <div className="flex items-center gap-4">
                    <div className="w-36 h-36 shrink-0" dir="ltr">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie
                            data={[
                              { name: 'جدّدوا بعد التذكير', value: renewed.length, color: '#0d9488' },
                              { name: 'لم يستجيبوا بعد', value: totalMessaged - renewed.length, color: '#e2e8f0' },
                            ]}
                            dataKey="value"
                            nameKey="name"
                            innerRadius={38}
                          ...
                          <Tooltip
                            ...
                            contentStyle={{ fontSize: 12, borderRadius: 8, direction: 'rtl', fontFamily: 'inherit' }}
                          />
                        </PieChart>
                      </ResponsiveContainer>
                    </div>
                    <div className="flex-1 space-y-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-slate-600">جدّدوا بعد التذكير</span>
                        </div>
                        <span className="text-xs font-bold text-teal-700">{renewed.length}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="w-2.5 h-2.5 rounded-full bg-slate-200 shrink-0" />
                          <span className="text-xs text-slate-600">لم يستجيبوا بعد</span>
                        </div>
                        <span className="text-xs font-bold text-slate-500">{totalMessaged - renewed.length}</span>
                      </div>
              ...
              {/* ── أكثر الأدوية شيوعاً — بطاقات مرتبة ── */}
              {topMeds.length > 0 && (
                <div className="bg-white border border-slate-200 rounded-xl p-5">
                  <p className="text-sm font-bold text-slate-800 mb-4">أكثر الأدوية شيوعاً عند مرضاك</p>
                    ...
                    {topMeds.map(([name, count], i) => (
                      <div key={name} className="flex items-center gap-4">
                        ...
                        <div className="text-right shrink-0">
                          <p className="text-lg font-black text-slate-900 tabular-nums leading-none">{count}</p>
                          <p className="text-[9px] text-slate-400 mt-0.5">{count === 1 ? 'مريض' : 'مرضى'}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {loyalPatients.length > 0 && (
                <div className="bg-white border border-slate-200 rounded-xl p-5">
                  <p className="text-sm font-bold text-slate-800 mb-4">أكثر مرضاك وفاءً</p>
                  <div className="space-y-4">
                  ...
                  <p className="text-xs text-slate-500 mt-4 pt-3 border-t border-slate-100 flex items-center gap-1.5">
                    <svg className="w-3.5 h-3.5 shrink-0 text-teal-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              ...
              {/* ── رسالة الختام ── */}
              {renewed.length > 0 ? (
                <div className="rounded-xl bg-teal-50 border border-teal-200 px-5 py-4 flex items-center justify-center gap-2.5">
                  ...
                  <p className="text-sm font-semibold text-teal-800 text-center">
                    <span className="text-lg font-black">{renewed.length}</span>{' '}مريض اختار صيدليتك مرة أخرى — هذا دليل على أن متابعتك تُحدث فرقاً.
                  </p>
                </div>
              ) : (
