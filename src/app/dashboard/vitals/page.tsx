...
export default function VitalsPage() {
  ...
  const [vitalsReportReviewed, setVitalsReportReviewed] = useState(false);
  const [vitalsSendApproving, setVitalsSendApproving] = useState(false);
...
مع تحيات ${pharmacyName} 💜`;
  ...
  const selectPatient = async (p: Patient, keepQuery = false) => {
    ...
    setVitalsReportReviewed(false); setSavedVitalsExclusions(''); setVitalsSendError('');
          ...
          if (planRes?.plan_id) {
            ...
            fetch('/api/weight-plan', {
              ...
              body: JSON.stringify({
                ...
                drug_allergies:       currentPatient.drug_allergies || [],
                food_allergies:       currentPatient.food_allergies || [],
                is_pregnant:          !!currentPatient.is_pregnant,
                is_lactating:         !!currentPatient.is_lactating,
                medications,
              ...
              <div className="p-5 space-y-4">
                ...
                {canShowNewPatient && (
                  <button onClick={() => setShowNewPatientModal(true)}
                    ...
                    className="w-full flex items-center justify-between gap-3 px-4 py-3 bg-slate-50 border border-dashed border-slate-300 hover:border-slate-900 hover:bg-white rounded-xl transition-all group disabled:opacity-50 disabled:cursor-not-allowed">
                  ...
                  <div className="rounded-xl border border-teal-200 bg-teal-50/40 overflow-hidden">
                    ...
                    {((currentPatient.drug_allergies?.length ?? 0) > 0 || (currentPatient.food_allergies?.length ?? 0) > 0 || currentPatient.is_pregnant || currentPatient.is_lactating) && (
                      <div className="px-4 pb-2.5 flex items-center gap-2 flex-wrap">
                        ...
                        {currentPatient.is_lactating && (
                          <span className="text-[10px] font-bold text-pink-700 bg-pink-50 border border-pink-200 px-2 py-1 rounded-lg">مرضعة</span>
                        )}
                        <span className="text-[10px] text-slate-400 mr-auto">يُعدَّل من السجل</span>
                      </div>
                    )}
            ...
            {currentPatient && !latestVisitId && (
              <div className="space-y-4 saas-slide-up">
                ...
                <AiActionButton
                  ...
                  size="lg"
                  onClick={() => handleSave()}
                  disabled={submitting || !hasAnyReading || !!softWarningMsg || readOnly}
                  title={readOnly ? READ_ONLY_MESSAGE : undefined}
                  loading={submitting}
                  ...
                  <div className="mx-5 my-5 border border-slate-200 rounded-xl divide-y divide-slate-200 overflow-hidden">
                    ...
                    <label className="flex items-start gap-2.5 px-4 py-3 bg-slate-50 cursor-pointer">
                      <input
                        ...
                        onChange={(e) => setVitalsReportReviewed(e.target.checked)}
                    ...
                    <div className="flex">
                      <button
                        ...
                        disabled={!vitalsReportReviewed || vitalsSendApproving || readOnly}
                        title={readOnly ? READ_ONLY_MESSAGE : undefined}
                ...
                {activeTests.weight && weightValue && bmiLive && weightStatus !== 'idle' && (
                  <div className="bg-white border border-purple-200 rounded-2xl shadow-sm overflow-hidden saas-slide-up">
                    ...
                    <div className="px-5 pt-4 pb-2" dir="ltr">
                      {(() => {
                        const pos = Math.min(Math.max(((bmiLive.value - 10) / 35) * 100, 1), 99);
                        ...
                        const dotHex = DOT_COLOR[bmiLive.dot] || '#0b0b0b';
                        return (
                          <>
                      ...
                      <div className="mx-5 my-5 border border-slate-200 rounded-xl divide-y divide-slate-200 overflow-hidden">
                        ...
                        <div className="flex">
                          <button
                            ...
                            disabled={!weightReviewed || !weightWaMsg || weightApproving || readOnly}
                            title={readOnly ? READ_ONLY_MESSAGE : undefined}
                            className="flex-[2] flex items-center justify-center gap-2 py-3 border-l border-slate-200 text-xs font-bold transition disabled:opacity-40 disabled:cursor-not-allowed bg-[#25D366] hover:bg-[#20BD5A] text-white cursor-pointer">
                            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
...
مع تحيات فريق ${pharmacyName} 💚`;
                      ...
                      disabled={!vitalsReportReviewed || !weightReviewed || readOnly}
                      title={readOnly ? READ_ONLY_MESSAGE : undefined}
                    ...
                    {(!vitalsReportReviewed || !weightReviewed) && (
                      <p className="text-[10px] text-amber-600 font-bold mt-2 text-center">أكّد مراجعة القسمين أعلاه أولاً لتفعيل هذا الزر</p>
                    )}
                  </div>
                )}
