      ...
      <main className="max-w-2xl mx-auto px-4 pt-5 space-y-4">
              ...
              <div className="bg-white border-2 border-slate-900 rounded-2xl shadow-sm overflow-hidden">
                        ...
                        <div className="flex-1">
                          {prod.product ? (
                            <div className="flex items-center gap-2 flex-wrap">
                              <p className="text-base font-bold text-slate-900">{prod.product.product_name}</p>
                              <span className="text-sm font-bold text-slate-700 tabular-nums">{prod.product.price} د.أ</span>
                              <span className="text-xs font-bold text-teal-700 bg-teal-50 border border-teal-200 rounded-lg px-2.5 py-0.5">
                                متوفر في الصيدلية
                              </span>
                            </div>
                          ) : (
                            <div>
                              <p className="text-base font-bold text-slate-900">{CATEGORY_LABELS[prod.category_code]}</p>
                              <p className="text-xs text-slate-400 mt-0.5">اسأل {pharmacyName} عن الخيارات المتاحة</p>
                            </div>
                          )}
                          <p className="text-sm text-slate-600 mt-1.5 leading-relaxed">{prod.reason}</p>
                          <p className="text-xs font-bold text-teal-700 mt-2 bg-teal-50 border border-teal-100 rounded-lg px-3 py-1.5 inline-block">
                            {prod.instruction}
                          </p>
                          {/* استفسار عن هذا المنتج تحديداً — يسجّل اهتمام المريض (حلقة التعلّم) ويفتح واتساب */}
                          {prod.product?.id && waPhone && (
                            <a
                              href={`https://api.whatsapp.com/send?phone=${waPhone}&text=${encodeURIComponent(`مرحباً، أنا ${patient.name} — عندي استفسار عن ${prod.product.product_name} المذكور في خطة إدارة وزني.\n\nرابط خطتي:\n${typeof window !== 'undefined' ? window.location.href : ''}`)}`}
                              ...
                              onClick={() => {
                                ...
                                fetch('/api/catalog/inquiry', {
                                  ...
                                  body: JSON.stringify({ context: 'weight_plan', context_id: planId, product_id: pid }),
                                  keepalive: true,
                                }).catch(() => {});
                              }}
                              className="flex w-fit items-center gap-1.5 mt-2.5 px-3 py-1.5 rounded-lg border border-[#25D366] bg-white text-[#128C7E] text-xs font-bold hover:bg-[#25D366]/10 transition"
                            >
                              ...
                              اسأل {pharmacyName} عن هذا المنتج
                            </a>
                          )}
                        </div>
                      </div>
                ...
                {waPhone && (
                  <div className="px-5 pb-5 pt-2">
                    <a href={`https://api.whatsapp.com/send?phone=${waPhone}&text=${encodeURIComponent(`مرحباً، أنا ${patient.name} 👋 شفت المقترحات في خطة إدارة وزني وعندي استفسار عنها.\n\nرابط زيارتي:\n${typeof window !== 'undefined' ? window.location.href : ''}`)}`} target="_blank" rel="noreferrer"
                      className="w-full flex items-center justify-center gap-2.5 py-3.5 bg-[#25D366] hover:bg-[#20BD5A] text-white rounded-xl text-[15px] font-semibold transition shadow-sm active:scale-[0.98]" style={{ letterSpacing: '-0.01em' }}>
        ...
        <div className="text-center pt-2 pb-4">
          <p className="text-xs text-slate-600 mb-3">
            أُعدّت نتائج التحليل بواسطة {pharmacyName}{performedBy ? `، د. ${performedBy}` : ''}
          </p>
          <AppFooter />
        </div>
      </main>
